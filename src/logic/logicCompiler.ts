...
const {
  anchorLabelCheck,
} = require('./aria-standards/critical/anchor-labels.js');
const {
  areaAltTextCheck,
} = require('./aria-standards/critical/area-maps-alt-text.js');
const { ariaHiddenCheck } = require('./aria-standards/critical/aria-hidden.js');
const {
  discernibleButtonTextCheck,
} = require('./aria-standards/critical/button-text.js');
const { imageAltsCheck } = require('./aria-standards/critical/img-alt-text.js');
const {
  inputButtonCheck,
} = require('./aria-standards/critical/input-button.js');
const {
  metaEquivRefreshCheck,
} = require('./aria-standards/critical/meta-http-equiv-refresh.js');
const {
  metaViewportCheck,
} = require('./aria-standards/critical/meta-viewport-text-zoom.js');
const {
  selectHasAccessNameCheck,
} = require('./aria-standards/critical/select-name.js');
const { uniqueIDsCheck } = require('./aria-standards/critical/unique-ids.js');
const {
  videoCaptionsCheck,
} = require('./aria-standards/critical/video-captions.js');
const {
  formsHaveLabelsCheck,
} = require('./aria-standards/critical/form-labels.js');
const { checkAriaRoles } = require('./aria-standards/critical/aria-roles.js');

export interface AriaRecommendations {
  [key: string]: any;
}

export function compileLogic(activeEditor: vscode.TextEditor) {
  ...
  // button-text
  ariaRecommendations.discernibleButtonText = discernibleButtonTextCheck(
    tag('button')
  );
  ...
  ariaRecommendations.metaViewport = metaViewportCheck(
    tag('meta[name="viewport"]')
  );

  // select-name
  ariaRecommendations.selectHasAccessName = selectHasAccessNameCheck(
    tag('select')
  );
  ...
  for (const key in ariaRecommendations) {
    if (
      Array.isArray(ariaRecommendations[key]) &&
      ariaRecommendations[key].length === 0
    ) {
